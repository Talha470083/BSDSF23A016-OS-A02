 ...
 * - preserves colorized output and column layout from v1.5.0
 * - adds -R option for recursive directory traversal
 * - uses lstat() to avoid following symlinks (prevents infinite loops)
 * - prints directory headers like `ls -R`
 *
 * Build:
 *   mkdir -p bin
...
static void do_ls(const char *dirpath, bool color_enabled, int recursive_flag) {
    ...
    /* print names in down-then-across order */
    ...
    if (recursive_flag) {
        for (size_t i = 0; i < count; ++i) {
            char fullpath[PATH_MAX];
            if (snprintf(fullpath, sizeof(fullpath), "%s/%s", dirpath, names[i]) >= (int)sizeof(fullpath))
                continue;

            struct stat st;
            if (lstat(fullpath, &st) == 0 && S_ISDIR(st.st_mode)) {
                do_ls(fullpath, color_enabled, recursive_flag);
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    do_ls(target, color_enabled, recursive_flag);
    return 0;
}
